...
use crate::error::{Error as OclError, ErrorKind as OclErrorKind, Result as OclResult};
use crate::standard::{
...
pub enum KernelError {
    ...
    CmdNoQueue,
    #[fail(
        display = "Global Work Size cannot be left unspecified. Set a default for the kernel or \
                   specify one when enqueuing command."
    )]
    CmdNoGws,
    #[fail(
        display = "Unable to resolve argument named: '{}'. Ensure that an argument with that name \
                   has been declared before building kernel.",
        _0
    )]
    NamedArgsInvalidArgName(String),
    #[fail(
        display = "No named arguments have been declared. Declare named arguments when before \
                   building kernel."
    )]
    NamedArgsNone,
    #[fail(
    ...
    #[fail(
        display = "Kernel argument type mismatch. The argument named: '{}' at index: [{}] should \
                   be a '{}' ({:?}).",
    ...
    BuilderNoKernelName,
    #[fail(
        display = "The wrong number of kernel arguments have been specified (required: {}, \
                   specified: {}). Use named arguments with 'None' or zero values to declare \
                   arguments you plan to assign a value to at a later time.",
        required, specified
    )]
...
where
    T: OclPrm,
{
    pub val: ArgValKeeper<'b>,
    pub type_id: Option<TypeId>,
    pub mem: Option<MemCore>,
        ...
        for device in devices {
            ...
            if !device.vendor().unwrap().contains("NVIDIA") {
                f.debug_struct("WorkGroup")
                    ...
                    // .field("GlobalWorkSize", &self.wg_info(device,
...
pub mod arg_type {
    ...
    impl ArgType {
        ...
        pub fn from_kern_and_idx(core: &KernelCore, arg_idx: u32) -> OclResult<ArgType> {
            use crate::core::{EmptyInfoResultError, ErrorKind as OclCoreErrorKind};
