...
use std::{env, fmt, iter, mem, ptr, thread};
...
use crate::ffi::{
    self, c_void, cl_addressing_mode, cl_bool, cl_buffer_create_type, cl_command_queue,
    ...
    cl_program_info, cl_sampler, cl_sampler_info, cl_uint, size_t,
};
...
pub extern "C" fn _complete_user_event(
    ...
    if event_status == CommandExecutionStatus::Complete as i32 && !user_data.is_null() {
        ...
        unsafe {
            ...
            println!(
                "::_complete_user_event: Setting event complete for: source: {:?}, target: {:?}...",
        ...
        #[cfg(feature = "event_debug_print")]
        println!(
            "  - Event status has been set to 'CommandExecutionStatus::Complete' for event: {:?}",
            tar_event_ptr
        );
    } else {
        ...
        match CommandExecutionStatus::from_i32(event_status) {
            Some(status_enum) => panic!(
                "ocl_core::_complete_event: User data is null or event is not complete. Status: \
                 '{:?}'",
                status_enum
            ),
            None => eval_errcode(
...
impl ApiError {
    pub fn new<S: Into<String>>(
        ...
        let status = match Status::from_i32(errcode) {
            Some(s) => s,
            None => panic!(
                "ocl_core::Error::err_status: Invalid error code: '{}'. Aborting.",
                errcode
            ),
        };
...
#[derive(Debug, Fail)]
#[fail(
    display = "OpenCL ({:?}) version too low to use {:?} (detected: {}, required: {}).",
...
pub enum ApiWrapperError {
    #[fail(
        display = "Unable to get platform id list after {} seconds of waiting.",
        _0
    )]
    ...
    #[fail(
        display = "One or more of the devices contained in the list provided to \
    ...
    #[fail(
        display = "Length of 'devices' must equal the length of 'binaries' (e.g. one binary per \
                   device)."
    )]
    CreateProgramWithBinaryDevicesLenMismatch,
    #[fail(
        display = "The specified function does not exist for the implementation or 'platform' is \
    ...
    #[fail(
        display = "The default platform set by the environment variable \
                   'OCL_DEFAULT_PLATFORM_IDX' has an index which is out of range (index: [{}], \
                   max: [{}]).",
        default_platform_idx, max_idx
    )]
    ...
    #[fail(
        display = "The default device type set by the environment variable \
                   'OCL_DEFAULT_DEVICE_TYPE': ('{}') is invalid. Valid types are: 'DEFAULT', \
                   'CPU', 'GPU', 'ACCELERATOR', 'CUSTOM', and 'ALL'.",
        _0
    )]
...
pub fn get_gl_context_info_khr(
    ...
    let cl_get_gl_context_info_khr_fn = unsafe {
        ...
        let plat = match properties.get_platform() {
            ...
            None => {
                return Err(
                    "ocl::core::get_gl_context_info_khr: Context properties must specify a \
                     platform."
                        .into(),
                );
            }
        ...
        if fn_ptr.is_null() {
            return Err(
                "Unable to get extension function address for clGetGLContextInfoKHR. The function \
...
pub fn compile_program<D: ClDeviceIdPtr>(
    ...
    assert!(
        input_headers.len() == header_include_names.len(),
        "ocl::core::compile_program(): Length of input_headers and header_include_names should be \
         equal."
    );
    ...
    M: AsMem<T> + MemCmdRw,
{
    ...
    if false {
        ...
        println!(
            "buffer_row_pitch_bytes: {}, buffer_slc_pitch_bytes: {}, host_row_pitch_bytes: {}, \
...
pub unsafe fn enqueue_kernel<En: ClNullEventPtr, Ewl: ClWaitListPtr>(
    ...
    #[cfg(feature = "kernel_debug_print")]
    print!(
        "core::enqueue_kernel('{}': work_dims: {}, gwo: {:?}, gws: {:?}, lws: {:?}, \
         wait_list_len: {}, wait_list_ptr: {:?}, new_event_ptr: {:?}) ",
...
pub fn event_status<'e, E: ClEventPtrRef<'e>>(
    ...
    CommandExecutionStatus::from_i32(status_int)
        .ok_or_else(|| OclCoreError::from("Error converting 'clGetEventInfo' status output."))
}
